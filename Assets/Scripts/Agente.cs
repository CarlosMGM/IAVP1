...
{
	/// <summary>
	///     La clase Agente es responsable de modelar los agentes y gestionar todos los comportamientos asociados para
	///     combinarlos (si es posible)
	/// </summary>
	public class Agente : MonoBehaviour
	{
		...
		public bool mezclarPorPeso;

		/// <summary>
		///     Mezclar por prioridad
		/// </summary>
		[Tooltip("Mezclar por prioridad.")]
		public bool mezclarPorPrioridad;
		...
		public virtual void FixedUpdate()
		{
			...
			var displacement = velocidad * Time.fixedDeltaTime;
			...
			var orientationVector = OriToVec(orientacion);
		...
		public virtual void Update()
		{
			...
			// ... c�digo previo
			var desplazamiento = velocidad * Time.deltaTime;
		...
		public void LateUpdate()
		{
			...
			if (rotacion > rotacionMax) rotacion = rotacionMax;

			if (Math.Abs(direccion.angular) < 0.1f) rotacion = 0.0f;

			if (Math.Abs(direccion.lineal.sqrMagnitude) < 0.1f) velocidad = Vector3.zero;
		...
		{
			if (!grupos.ContainsKey(prioridad)) grupos.Add(prioridad, new List<Direccion>());
		...
		private Direccion GetPrioridadDireccion()
		{
			...
			var gIdList = new List<int>(grupos.Keys);
			gIdList.Sort();
			foreach (var gid in gIdList)
			{
				direccion = new Direccion();
				foreach (var direccionIndividual in grupos[gid])
				{
